...
// Antes de enviar para a API, limpamos espaços e transformamos campos vazios.
...
export function LocationsPage() {
  ...
  // Filtros e paginação controlam quais localizações a API deve devolver.
  ...
  // Debounce da busca: só chamamos a API 400ms depois da última tecla.
  useEffect(() => {
  ...
  // Recarrega lista e resumo depois de qualquer alteração de dados.
  async function reloadLocations() {
    await Promise.all([locationListQuery.reload(), locationSummaryQuery.reload()])
  }

  // Salva criação ou edição e atualiza a tela.
  async function handleSubmitLocationForm(values: LocationFormValues) {
    ...
    try {
      if (formMode === 'edit' && locationInForm) {
        await updateLocation.update({ locationId: locationInForm.id, payload })
  ...
  // Salva a nova situação via PATCH e atualiza a tela.
  ...
  // Exclui o local; a API bloqueia com 409 se houver equipamento vinculado.
